...
"$EMQX_BIN" ctl plugins stop emqx_unsgov-1.0.0 >/dev/null 2>&1 || true
"$EMQX_BIN" ctl plugins uninstall emqx_unsgov-1.0.0 >/dev/null 2>&1 || true
"$EMQX_BIN" eval "emqx_hooks:del('message.publish', {emqx_unsgov, on_message_publish}), ok." >/dev/null 2>&1 || true
rm -rf "$ROOT_DIR/_build/emqx-enterprise/rel/emqx/plugins/emqx_unsgov-1.0.0" || true
rm -f "$ROOT_DIR/_build/emqx-enterprise/rel/emqx/plugins/emqx_unsgov-1.0.0.tar.gz" || true
./scripts/run-plugin-dev.sh emqx_unsgov
