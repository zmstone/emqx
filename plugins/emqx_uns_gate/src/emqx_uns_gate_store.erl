...
-define(TOPIC_INDEX_TAB, emqx_uns_gate_topic_index).
...
apply_put_local(Model, Activate) ->
    gen_server:cast(?MODULE, {runtime_apply_put, Model, Activate}),
    ok.

apply_delete_local(Id) ->
    gen_server:cast(?MODULE, {runtime_apply_delete, to_bin(Id)}),
    ok.

apply_activate_local(Id, Model) ->
    gen_server:cast(?MODULE, {runtime_apply_activate, to_bin(Id), Model}),
    ok.

apply_deactivate_local(Id) ->
    gen_server:cast(?MODULE, {runtime_apply_deactivate, to_bin(Id)}),
    ok.
...
do_delete_model(Id, State0) ->
    case get_model_record(Id) of
        {ok, _Record} ->
            ...
            ok = emqx_uns_gate_metrics:delete_model(Id),
...
do_validate_topic(Topic) ->
    case select_screened_model(Topic) of
        undefined ->
            case has_active_models() of
                true -> {deny, topic_nomatch, []};
                false -> {allow, undefined}
            end;
        Selected ->
...
do_validate_topic_selected({Id, Compiled}, Topic) ->
    case emqx_uns_gate_model:validate_topic(Compiled, Topic) of
        allow ->
            {allow, Id};
        {deny, Reason} ->
...
do_validate_message_selected({Id, Compiled}, Topic, Payload, ValidatePayload) ->
    case emqx_uns_gate_model:validate_message(Compiled, Topic, Payload, ValidatePayload) of
        allow ->
            {allow, Id};
        {deny, Reason} ->
            {deny, Reason, [{Id, Reason}]}
...
select_screened_model(Topic) ->
    Matches0 = topic_index_matches(Topic),
    Matches =
        case Matches0 of
            [] ->
                %% Keep "not_endpoint" behavior: a short topic can still screen-in
                %% a model if it is a prefix of an indexed endpoint filter.
                topic_index_matches_filter(<<Topic/binary, "/#">>);
            _ ->
                Matches0
        end,
    ...
    {LoadedActiveIds, FilterOwner0} =
        lists:foldl(
            fun(Record, {ActiveAcc, OwnerAcc}) ->
                Id = model_id(Record),
                case emqx_uns_gate_model:compile(model_raw(Record)) of
                    {ok, Compiled} ->
                        ...
                        case lists:member(Id, ActiveIds0) of
                            true ->
                                case check_filter_conflict(Compiled, Id, OwnerAcc) of
                                    ...
                                    {error, {conflicting_topic_filter, Filter, ExistingId}} ->
                                        ?LOG(error, #{
                                            msg =>
                                                "skip_active_model_on_boot_due_to_filter_conflict",
                                            model_id => Id,
                                            conflicting_filter => Filter,
                                            existing_model_id => ExistingId
                                        }),
                                        {ActiveAcc, OwnerAcc}
                                end;
                            false ->
            ...
            case Acc of
                ok ->
                    ...
                    case maps:get(Filter, FilterOwner, undefined) of
                        undefined -> ok;
                        SelfId -> ok;
                        ExistingId -> {error, {conflicting_topic_filter, Filter, ExistingId}}
                    end;
                Error ->
                    Error
            end
...
topic_index_matches_filter(TopicFilter) ->
    try emqx_topic_index:matches_filter(TopicFilter, ?TOPIC_INDEX_TAB, [unique]) of
        Matches -> Matches
    catch
        error:badarg -> []
    end.
