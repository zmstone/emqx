...
handle_unsubscribe(_UnsubscribeType, _UnsubscribeCtx, Handler0, _TopicFilter) ->
    ok = maybe_delete_cursor(Handler0),
    Handler0#h{cursor = ?done, nudge_enqueued = false}.

handle_terminate(Handler0) ->
    ok = maybe_delete_cursor(Handler0).
...
maybe_delete_cursor(#h{cursor = ?cursor(Cursor), mod = Mod, state = State}) ->
    _ = Mod:delete_cursor(State, Cursor),
    ok;
maybe_delete_cursor(_Handler) ->
    ok.
