...
handle(get, [<<"model">>], _Request) ->
    case emqx_unsgov_store:active_model() of
        {ok, Entry} ->
            {ok, 200, #{}, Entry};
        {error, not_found} ->
...
prometheus_metrics(Stats) ->
    GlobalMetrics = [
        {<<"emqx_unsgov_messages_total">>, maps:get(messages_total, Stats, 0)},
        {<<"emqx_unsgov_messages_allowed_total">>, maps:get(messages_allowed, Stats, 0)},
        {<<"emqx_unsgov_messages_dropped_total">>, maps:get(messages_dropped, Stats, 0)},
        {<<"emqx_unsgov_topic_nomatch_total">>, maps:get(topic_nomatch, Stats, 0)},
        {<<"emqx_unsgov_topic_invalid_total">>, maps:get(topic_invalid, Stats, 0)},
        {<<"emqx_unsgov_payload_invalid_total">>, maps:get(payload_invalid, Stats, 0)},
        {<<"emqx_unsgov_not_endpoint_total">>, maps:get(not_endpoint, Stats, 0)},
        {<<"emqx_unsgov_exempt_total">>, maps:get(exempt, Stats, 0)},
        {<<"emqx_unsgov_uptime_seconds">>, maps:get(uptime_seconds, Stats, 0)}
    ],
    PerModel = maps:get(per_model, Stats, #{}),
    [
        [format_help_and_type(Name) || {Name, _} <- GlobalMetrics],
        [format_metric(Name, Value) || {Name, Value} <- GlobalMetrics],
        format_help_and_type(<<"emqx_unsgov_model_messages_total">>),
        format_help_and_type(<<"emqx_unsgov_model_messages_allowed_total">>),
        format_help_and_type(<<"emqx_unsgov_model_messages_dropped_total">>),
        format_help_and_type(<<"emqx_unsgov_model_topic_invalid_total">>),
        format_help_and_type(<<"emqx_unsgov_model_payload_invalid_total">>),
        format_help_and_type(<<"emqx_unsgov_model_not_endpoint_total">>),
...
format_help_and_type(Name) ->
    [
        <<"# HELP ">>,
        Name,
        <<" UNS Governance metric\n">>,
        <<"# TYPE ">>,
        Name,
        <<" counter\n">>
    ].
