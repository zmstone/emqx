...
# Be specific: don't catch the substring "rejected" alone because git uses
# "[remote rejected]" for ALL push failures (e.g. branch-protection denial)
...
if grep -qE 'non-fast-forward|fetch first' "$PUSH_ERR"; then
    ...
    echo "::error::${RELEASE_BRANCH} push rejected by branch protection. Add the bot identity (AUTH_APP_ID) as a bypass principal on the ${RELEASE_BRANCH} branch protection rule."
fi
exit 1
