...
validate_topic(Compiled, Topic) ->
    emqx_uns_gate_model_topic:validate_topic(Compiled, Topic).

validate_message(Compiled, Topic, Payload, ValidatePayload) ->
    emqx_uns_gate_model_message:validate_message(
...
compile_var_types(Map) when is_map(Map) ->
    maps:fold(
        fun(K, V0, Acc) ->
            ...
            Acc#{VK => emqx_uns_gate_model_schema:compile_var_matcher(V)}
        end,
        #{},
        Map
    );
...
compile_variable(Key, Node, VarTypes, PayloadTypes) ->
    case variable_name(Key) of
        error ->
            {error, {invalid_variable_key, Key}};
        VarName ->
            case variable_matcher(Key, VarName, Node, VarTypes) of
                {error, _} = Error ->
                    Error;
                {VarTypeName, Matcher} ->
                    case read_map(Node, [children], undefined) of
