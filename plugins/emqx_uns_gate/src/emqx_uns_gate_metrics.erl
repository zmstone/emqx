...
-define(COUNTERS_TAB, emqx_uns_gate_metrics_counters).
-define(RECENT_DROPS_TAB, emqx_uns_gate_metrics_recent_drops).
...
merge_node_stats(NodeStats) ->
    Merged0 =
        lists:foldl(
            fun(Stats, Acc) ->
                Acc#{
                    messages_total => maps:get(messages_total, Acc, 0) +
                        maps:get(messages_total, Stats, 0),
                    messages_allowed => maps:get(messages_allowed, Acc, 0) +
                        maps:get(messages_allowed, Stats, 0),
                    messages_dropped => maps:get(messages_dropped, Acc, 0) +
                        maps:get(messages_dropped, Stats, 0),
                    topic_nomatch => maps:get(topic_nomatch, Acc, 0) +
                        maps:get(topic_nomatch, Stats, 0),
                    topic_invalid => maps:get(topic_invalid, Acc, 0) +
                        maps:get(topic_invalid, Stats, 0),
                    payload_invalid => maps:get(payload_invalid, Acc, 0) +
                        maps:get(payload_invalid, Stats, 0),
                    not_endpoint => maps:get(not_endpoint, Acc, 0) +
                        maps:get(not_endpoint, Stats, 0),
                    exempt => maps:get(exempt, Acc, 0) + maps:get(exempt, Stats, 0),
    ...
    Drops = lists:sublist(Drops1, ?MAX_RECENT_DROPS),
    TopicNoMatch = maps:get(topic_nomatch, Merged0, 0),
    TopicInvalid = maps:get(topic_invalid, Merged0, 0),
    PayloadInvalid = maps:get(payload_invalid, Merged0, 0),
    NotEndpoint = maps:get(not_endpoint, Merged0, 0),
    Merged0#{
        drop_breakdown => #{
            topic_nomatch => TopicNoMatch,
            topic_invalid => TopicInvalid,
...
merge_per_model(Left, Right) ->
    maps:fold(
        fun(ModelId, StatsR, Acc0) ->
            ...
            Acc0#{
                ModelId => #{
                    messages_total => maps:get(messages_total, StatsL, 0) +
                        maps:get(messages_total, StatsR, 0),
                    messages_allowed => maps:get(messages_allowed, StatsL, 0) +
                        maps:get(messages_allowed, StatsR, 0),
                    messages_dropped => maps:get(messages_dropped, StatsL, 0) +
                        maps:get(messages_dropped, StatsR, 0),
                    topic_invalid => maps:get(topic_invalid, StatsL, 0) +
                        maps:get(topic_invalid, StatsR, 0),
                    payload_invalid => maps:get(payload_invalid, StatsL, 0) +
                        maps:get(payload_invalid, StatsR, 0),
                    not_endpoint => maps:get(not_endpoint, StatsL, 0) +
                        maps:get(not_endpoint, StatsR, 0)
                }
            }
        end,
        Left,
