...
t_cleanup_after_retain(_) ->
    ...
    ClientId = <<"clientid">>,
    ClientId2 = <<"clientid2">>,
    emqx_cm_registry:register_channel({{?GBNS, ClientId}, Pid}),
    emqx_cm_registry:register_channel({{?GBNS, ClientId2}, Pid}),
    ...
    ok = emqx_cm:clean_down({Pid, {?GBNS, ClientId}}),
    ok = emqx_cm:clean_down({Pid, {?GBNS, ClientId2}}),
...
t_count_cache(_) ->
    ...
    Channels = lists:map(fun(ClientId) -> {{?GBNS, ClientId}, Pid} end, ClientIds),
    lists:foreach(
    ...
    emqx_cm_registry:register_channel({{?GBNS, <<"-1">>}, Pid}),
